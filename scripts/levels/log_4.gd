...
@export var speed: int = 16  # Vitesse du big car
...
func _physics_process(_delta: float) -> void:
	set_velocity(vel)
	move_and_slide()
	if position.x > get_viewport_rect().size.x + 4*sprite_size:  # Si le big car sort de l'écran à droite
		position.x = -4*sprite_size