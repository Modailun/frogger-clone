...
@export var speed: int = 128  # Vitesse du big car
...
func _ready() -> void:
	add_to_group("big_car")

func _physics_process(delta: float) -> void:
	position.x += direction * speed * delta
	if position.x > get_viewport_rect().size.x + 2*sprite_size:  # Si le big car sort de l'écran à droite
